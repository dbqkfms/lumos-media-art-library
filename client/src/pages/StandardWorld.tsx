    ...
    >
      <div className="relative overflow-hidden aspect-video bg-[#111]">
        ...
        <div className="absolute bottom-3 left-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="font-accent text-[9px] tracking-[0.24em] text-[#D4A843] bg-black/50 backdrop-blur-sm px-2.5 py-1 border border-[#D4A843]/20">
            HOVER PREVIEW
          </div>
        </div>
      </div>
...
export default function StandardWorld() {
  ...
  const standardArtworks = useMemo(
    () => artworks.filter((artwork) => artwork.worldType === "standard"),
    [artworks],
  );

  const standardCategories = useMemo(
    () => deriveCategories(artworks, "standard"),
    [artworks],
  );
  ...
  const filteredArtworks = useMemo(() => {
    ...
    return result;
  }, [activeFilter, searchQuery, standardArtworks]);
    ...
    <div className="min-h-screen bg-transparent">
      ...
      <section className="relative h-[65vh] flex items-end justify-start overflow-hidden">
        ...
        <div className="relative z-10 px-12 md:px-20 pb-16 md:pb-20">
          <p className="font-accent text-[10px] tracking-[0.6em] text-[#D4A843] mb-5">
            PREMIUM MEDIA ART
          </p>
          ...
          <p className="text-lg text-[#aaaaaa] font-light max-w-xl text-shadow-medium tracking-wide">
            Premium cinematic media art for gallery and signature display spaces.
          </p>
        </div>
      </section>
          ...
          <div className="flex flex-wrap gap-2.5 mb-12">
            {standardCategories.map((category) => (
              <button
                ...
                className={`filter-pill ${activeFilter === category ? "filter-pill-active-standard" : ""}`}
              >
                {category}
              </button>
            ))}
      ...
      <section className="py-36 px-12 md:px-20 bg-[#030303] border-t border-white/5 relative z-10">
        <div className="max-w-3xl">
          <p className="font-accent text-[10px] tracking-[0.6em] text-[#D4A843] mb-6">
            CONTACT
          </p>
          ...
          <p className="text-lg text-[#aaaaaa] font-light mb-12 leading-[2] max-w-xl tracking-wide">
            LUMOS STANDARD는 갤러리, 호텔, 프리미엄 상업 공간을 위한 시네마틱 미디어아트 컬렉션입니다.
          </p>
